...
import { getKeywordByPosition, getKeywordOrigin, searchKeywordLocation, getAllKeywordReferences } from '../../helper/KeywordHelper';
...
export class RobotRenameProvider implements RenameProvider {
    ...
    private static renameKey(document: TextDocument, keyword: string[], newName: string)
        ...
        if (keyword.length == 1) {
            ...
            keywordOrigin = searchKeywordLocation(document, keyword[0], keyword[1]);
            key = keyword[1];
        }
        else {
            return null;
